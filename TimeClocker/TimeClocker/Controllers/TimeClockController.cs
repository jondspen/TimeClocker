            ...
            for (int i = 0; i < prunedTimeList.Count; i++)
            {
                if (prunedTimeList[i].IsClockIn)
                {
                    ...
                    if ((i + 1) == prunedTimeList.Count)
                    {
                        ...
                        currentTimeTotals = new TimeTotals() { InTime = currentTimeTotals.InTime, OutTime = DateTime.Now, RunningTotal = (DateTime.Now.Subtract(currentTimeTotals.InTime)).TotalHours };
